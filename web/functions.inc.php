	...
	foreach ($siteInfo->pages as $pi) {
		...
		if (is_array($pi['alias'])) {
			if ($lang && isset($pi['alias'][$lang])) {
				return (!$siteInfo->modRewrite && !preg_match('#^\.\.\/\?route=#', $pi['alias'][$lang]) ? '../?route=' : '').
						(strpos($pi['alias'][$lang], '#') === false ? $lang : '').
						(rtrim($pi['alias'][$lang], '/') ? '/' : '').rtrim($pi['alias'][$lang], '/').'/';
			} else if ($siteInfo->defLang && isset($pi['alias'][$siteInfo->defLang])) {
				return (!$siteInfo->modRewrite && !preg_match('#^\.\.\/\?route=#', $pi['alias'][$siteInfo->defLang]) ? '../?route=' : '').
						(strpos($pi['alias'][$lang], '#') === false ? $siteInfo->defLang : '').
						(rtrim($pi['alias'][$siteInfo->defLang], '/') ? '/' : '').rtrim($pi['alias'][$siteInfo->defLang], '/').'/';
			}
		} else {
